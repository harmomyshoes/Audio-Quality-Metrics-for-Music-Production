...
{
   echo ""
   echo "Usage: $0 -a parameterA -b parameterB -r parameterC -m parameterM"
   echo -e "\t-a The Audio File Path"
   echo -e "\t-b The bitrate applied by LAME"
   echo -e "\t-r Reference file address"
   echo -e "\t-m PEAQ model, 'basic' or 'advanced'"
...
CalcPeakResult()
{
   ...
   peaq --$parameterM $parameterC $outputMp3toWavfilepath;
   ####peaq --basic $parameterA $outputrsWavfilepath;
}


while getopts "a:b:r:m:" opt
do
   case "$opt" in
      a ) parameterA="$OPTARG" ;;
      b ) parameterB="$OPTARG" ;;
...
if [ -z "$parameterA" ] || [ -z $parameterB ] || [ -z $parameterC ] || [ -z $parameterM ]
then
