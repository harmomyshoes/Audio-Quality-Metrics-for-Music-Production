...
class SingleFileAudioMixer:
    ...
    def __init__(self, foldpath, filename="", isMONO=False, StartingTime=0, TrackType = NEUtil.MixingType.File):
        self.Foldpath = foldpath
        self.isMONO = isMONO
        self.OutputMixingFold = foldpath +'/Mixing_Result/'
        self.StartingTime = StartingTime
        ...
        if (os.path.isfile(foldpath+"/"+filename)&(TrackType == NEUtil.MixingType.File)):
            self.InitalData,self.SampleRate,self.isMONO = self.LoadSingleFile(filename,foldpath,isMONO, StartingTime)
        else:
